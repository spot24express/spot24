...
React 18 · TypeScript (estricto) · Vite 5 · Tailwind 3 (tokens de marca) · Firebase 12 (Auth, Firestore, App Check, FCM; Storage opcional) · TanStack Query 5 · Zustand 5 · react-router 6 · vite-plugin-pwa (Workbox) · Netlify (hosting + funciones de servidor).
...
functions/                # Lógica de negocio (cores): orden, BCV, pagos, stock, roles
netlify/functions/        # Adaptadores HTTP (modo Lite, plan gratis) que ejecutan los cores
```

**Regla dura:** ningún componente toca Firestore/Storage directo — siempre la capa `services/` de su módulo. Ninguna operación sensible en cliente: crear orden, calcular montos, verificar pago, ajustar stock y asignar roles viven en funciones de servidor. Cada core (`functions/src/*`) tiene dos frentes: wrapper `onCall` (Cloud Functions, plan Blaze) y adaptador HTTP en `netlify/functions/` (plan Spark gratis, el modo por defecto).
...
```bash
cd functions && npm install && npm run build && npm test   # cores + pruebas
npm run typecheck:netlify                                   # adaptadores HTTP
```

Las imágenes de producto viven en `public/img/products/` (servidas por el CDN de Netlify); el panel admin las asigna por ruta/URL. Storage de Firebase es opcional (solo plan Blaze): sin bucket, la verificación de pagos funciona por referencia y la UI oculta las subidas de archivos.

## Despliegue

Guía completa en **[docs/DESPLIEGUE.md](docs/DESPLIEGUE.md)**: Firebase plan Spark (gratis, sin tarjeta), App Check con reCAPTCHA v3, service account para Netlify, variables de entorno, reglas e índices, y checklist de verificación con `curl`.

## Seguridad

- Reglas Firestore/Storage **deny-by-default** con suite de pruebas (`src/tests/rules/`).
- App Check (reCAPTCHA v3 gratis) en monitoreo y luego Enforce en Firestore/Functions.
- El cliente nunca escribe en `orders` — solo la función `fn-createOrder`, idempotente por clave.
...
- Datos sensibles cifrados con **AES-256-GCM** en reposo (`ENC_KEY_HEX` como variable de Netlify).
